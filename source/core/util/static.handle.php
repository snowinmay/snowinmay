...
class XHandle{
	...
	public static function getStrpos($s_str, $s_needlechar){
		if(empty($s_str)){return;}
		if(empty($s_needlechar)){return;}
	...
	public static function cutStrLen($string, $sublen, $start=0){ 
        ...
        if (OECMS_CHARSET == 'UTF-8' OR OECMS_CHARSET == 'utf-8'){
	...
	public static function forbidChar($string){
		$forbidchar = array('select','update','union','insert','load_file','outfile','where','char','concat','#');
	...
	public static function checkOdayChar($string){
		$checkflag = false;
		$forbidchar = array('select','update','union','insert','load_file','outfile','char','concat','#');
		if(self::isChar($string)){
			foreach($forbidchar as $key){
				if(strpos(strtolower($string),$key)){
					$checkflag = true;
