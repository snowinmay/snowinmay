...
class control extends adminbase {
    ...
    public function action_saveedit() {
        ...
        if (true === $result) {
            ...
            if (!empty($this->dir)) {
                XHandle::halt('模板文件修改成功', $this->cpfile.'?c=templet&a=edit&id='.urlencode($this->id).'&dir='.urlencode($this->dir).'', 0);
            }
            else {
                XHandle::halt('模板文件修改成功', $this->cpfile.'?c=templet&a=edit&id='.urlencode($this->id), 0);
            }
        }
        else {
