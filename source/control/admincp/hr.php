...
class control extends adminbase {
    ...
    public function action_run() {
		...
		TPL::display($this->cptpl.'hr.tpl');
	}
    
    
    public function action_add() {
        $this->checkAuth('hr_add');
        $this->_getItems();
    ...
    public function action_saveadd() {
        ...
        if (true === $result) {
            $this->log('hr_add', '', 1);
            XHandle::halt('添加成功', $this->cpfile.'?c=hr&a=add&tid='.$this->tid.'&scatid='.$args['catid'], 0);
        }
        else {
            $this->log('hr_add', '', 0);
            XHandle::halt('添加失败', '', 1);
    ...
    public function action_saveedit() {
        ...
        unset($model);
        if (true === $result) {
            $this->log('hr_edit', "id={$id}", 1);
            XHandle::halt('编辑成功', $this->cpfile.'?c=hr&'.$this->_comeurl.'', 0);
        }
        else {
            $this->log('hr_edit', "id={$id}", 0);
            XHandle::halt('编辑失败', '', 1);
        }
    }
    ...
    public function action_del() {
        ...
        if (true === $result) {
            $this->log('hr_del', "id={$array_id}", 1);
            XHandle::halt('删除成功', $this->cpfile.'?c=hr&tid='.$this->tid.'', 0);
        }
        else {
            $this->log('hr_del', "id={$array_id}", 0);
            XHandle::halt('删除失败', '', 1);
        }
    ...
    private function _validAdd() {
        $args = XRequest::getGpc(array(
            'treeid', 'catid', 'title', 'number', 'workarea',
            'thumbfiles', 'uploadfiles', 'summary', 'hrcontact',
