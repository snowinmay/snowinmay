...
class articleIModel extends X {
    ...
    public function getVolist($where='', $orderby='', $num=0, $limit='') {
        ...
        if (!empty($limit)) {
            ...
            $num = intval($num)<1 ? intval(parent::$cfg['articlenum']) : intval($num);
            $sql .= " LIMIT {$num}";
        }
        $data = parent::$obj->getall($sql);
    ...
    public function getOneData($id) {
        ...
        if (!empty($data)) {
            ...
            if ($data['linktype'] == 2) {
                ...
                $data['url'] = XUrl::getContentUrl('article', $data['dirname'], $data['articleid']);
            }
            unset($m_label);
            #图片处理
            ...
            $attr_data = $m_atrr->getAttrList('article', $id);
            $extend_data = $m_atrr->assembleAttr('article', $id);
            ...
            $data['content'] = $m_link->tagContent('article', $data['content']);
            unset($m_link);
        }
    ...
    public function getPrevious($treeid, $id) {
        ...
        $query = "SELECT v.*, c.dirname".
                " FROM ".DB_PREFIX."article AS v".
                " LEFT JOIN ".DB_PREFIX."category AS c ON v.catid=c.catid".
                " WHERE v.articleid<{$id} AND v.flag='1' AND v.treeid='{$treeid}'".
                " ORDER BY v.articleid DESC LIMIT 1";
        ...
        if (!empty($rows)) {
            ...
            if ($rows['linktype'] == 2) {
                ...
                $rows['url'] = XUrl::getContentUrl('article', $rows['dirname'], $rows['articleid']);
            }
        }
    ...
    public function getNext($treeid, $id) {
        ...
        $query = "SELECT v.*, c.dirname".
                ...
                " LEFT JOIN ".DB_PREFIX."category AS c ON v.catid=c.catid".
                " WHERE v.articleid>{$id} AND v.flag='1' AND v.treeid='{$treeid}'".
                " ORDER BY v.articleid ASC LIMIT 1";
        $rows = parent::$obj->fetch_first($query);
        if (!empty($rows)) {
            #URL路由
            if ($rows['linktype'] == 2) {
        ...
        if (!empty($data)) {
            ...
            foreach($data as $key=>$value) {            
                ...
                if ($value['catlinktype'] == 2) {
                    ...
                    $data[$key]['caturl'] = XUrl::getCategoryUrl('article', $value['catid'], $value['dirname']);
                }
                if ($value['linktype'] == 2) {
                    $data[$key]['url'] = $m_label->repLabel($value['linkurl']);
                }
                else {
                    $data[$key]['url'] = XUrl::getContentUrl('article', $value['dirname'], $value['articleid']);
                }
                unset($m_label);
                #图片处理
                ...
                #标题处理
                $data[$key]['sort_title'] = XHandle::cutStrLen($value['title'], parent::$cfg['articlelen']);
                $data[$key]['i'] = $i;
                $i = ($i+1); 
            }
        } 
    ...
    private function _updateHits($id) {
        parent::$obj->update(DB_PREFIX.'article', array('hits'=>'[[hits+1]]'), 'articleid='.$id.'');
    } 
